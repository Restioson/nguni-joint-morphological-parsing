...
from ..common import TaggingDataset, tune_model, train_all, \
    EmbedSingletonFeature, tokenize_into_morphemes, split_words, EmbedBySumming, tokenize_into_chars
...
feature_level = (
    ...
    tokenize_into_chars,
    lambda config, dset, dev: EmbedBySumming(dset, config["embed_target_embed"], dev)
)
...
def final_train():
    ...
    train_all(data_dir, model, splits, feature_level, cfg, device=torch.device("cpu"), langs=["ZU"], add_valid_to_training_set=False)
