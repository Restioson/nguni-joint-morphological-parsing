...
from annotated_corpus_dataset import TaggingDataset, SEQ_PAD_IX
...
class BiLstmCrfTagger(nn.Module):
    ...
    def __build_features(self, morphemes: torch.Tensor):
        """Build features using a bi-lstm for the CRF to then tag"""
        masks = (morphemes != SEQ_PAD_IX).any(dim=2)  # SEQ_PAD_IX is 0, so this checks which words have non-padding submorpheme IXs
